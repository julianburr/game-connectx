 ...
 *
 * Description:
 *		Manages queries and executions from and to the SQL database
 * 		Also provides other features like executing files, dumping tables, etc.
 **/
...
class SqlManager {
	...
	private function loadConfig($path=null){
		...
		if(is_file(__DIR__ . $this->configfile)){
			...
			throw new Exception("SQL config file not found! :: " . __DIR__ . $this->configfile);
		}
	...
	public function get($table,$keyfield,$keyvalue){
		...
		if(is_object($keyvalue)){
			throw new Exception("Ups! It's an object!");
		}
